...
class PomodoroTimer:
    def __init__(self): #UI Stuff
        self.root = tk.Tk()
        self.root.geometry("600x300")
        self.root.title("Pomodoro Timer")
        self.root.tk.call("wm", "iconphoto", self.root._w, PhotoImage(file="/Users/rkayyo/Documents/Portfolio Projects/Pomodoro/yinyang.png"))

        #using ttk need to define style cannot set font size for tab view
        #do it using configuration
        self.s = ttk.Style()
...
#Seperate Functions For Functionality 
#Pass keyword means do nothing allows us to leave functions unimplemented\
    ...
    def start_timer(self):
        ...
        if timer_id == 1:
            full_seconds = 60 * 25 #60 seconds * 25 minutes
            full_seconds = 5
            while full_seconds > 0 and not self.stopped:
                minutes, seconds = divmod(full_seconds, 60) #division in minutes remainder in seconds
                self.pomodoro_timer_label.config(text=f"{minutes:02d}:{seconds:02d}")
                self.root.update()
                time.sleep(1) #upgrade idea! allow usage of floats so no delay on button press
                full_seconds -= 1 #change both time.sleep(value) and full_seconds -= value to a floating point ie. 0.1
            ...
            while full_seconds > 0 and not self.stopped:
                ...
                time.sleep(1) 
                full_seconds -= 1
            if not self.skipped or self.skipped:
                self.tabs.select(0)
                self.start_timer()
